import ExcelJS, { FillPattern, CellFormulaValue, WorksheetViewFrozen, Buffer, ValueType } from "exceljs";
import { Sheet, addressToCell, CellConfig, cellAddress } from '@rowsncolumns/spreadsheet'
import { CellInterface, getBoundedCells, cellIdentifier, isNull } from "@rowsncolumns/grid";
 ...
 * @param param0 
 */
export const excelToSheets = async ({ file }: ExportProps): Promise<ExportResults> => {
  let resolver: (value: ExportResults) => void | null
  const sheetPromise: Promise<ExportResults> = new Promise(resolve => resolver = resolve)
    ...
    workbook.eachSheet((sheet, id) => {
      ...
      for (let i = 1; i <= rowCount; i++) {
        ...
        for (let j = 1; j <= columnCount; j++) {
          ...
          const currentCell : CellInterface = { rowIndex: rowId, columnIndex: j }
          /* Check if its a merged cell */
          const isMerged = isMergedCell(currentCell)
          if (isMerged) {
            const mergedBounds = mergedCellMap.get(cellIdentifier(currentCell.rowIndex, currentCell.columnIndex))
            if (mergedBounds.top !== currentCell.rowIndex || mergedBounds.left !== currentCell.columnIndex) {
              continue
            }
          }
          ...
          if (fillType !== void 0 && cell.style.fill?.type === 'pattern') {
            const fillValue = (cell.style.fill as FillPattern).bgColor?.argb?.slice(2)                
          ...
          for (const key in border) {                
            if (key === 'bottom') {
              ...
              strokes.strokeBottomColor = '#' + border[key]?.color?.argb?.slice(2)
            }
            if (key === 'top') {
            ...
            if (key === 'left') {
              ...
              strokes.strokeLeftColor = '#' + border[key]?.color?.argb?.slice(2)
            }
            if (key === 'right') {
              strokes.strokeRightWidth = 1
              strokes.strokeRightColor = '#' + border[key]?.color?.argb?.slice(2)
            }
          }              
          ...
          _sheet.cells[rowId][j] = {
            text: typeof value === 'object' || isNull(value) ? '' : value.toString(),
            fill,
            color,
            datatype,
            ...strokes,
 ...
 * @param sheets 
 */
export const createExcelFileFromSheets = async (sheets: Sheet[]): Promise<Buffer> => {
    ...
    for (let j = 1; j <= rowCount; j++) {
      ...
      for (let k = 1; k <= cellCount; k++) {
        ...
        if (address !== null) {
          ...
          const value = isNumber && cell.text !== void 0 ? parseFloat(cell.text.toString()) : cell.text
          if (value !== void 0) {
            newCell.value = value
          }
        }
